...
from _03dp_noise.noise_adder_Attacks import DPNoiseAdder 
...
def set_random_seed(seed_value=42):###Same as the non-attack situation
    """
...
def load_model_and_tokenizer(model_name):###Same as the non-attack situation
    """
...
def calculate_privacy_budget(args, risk_level):###Same as the non-attack situation
    return round(
        args.max_budget - (risk_level - 1) * ((args.max_budget - args.epsilon) / 4), 
        2
    )

def save_results(###Compared to non-attack situation, only need to calculate average attack success rate and save it
    all_results, 
    args, 
    pii_detection_times, 
    risk_budget_times, 
    ...
    ### If ablation_1 to ablation_6 are all False, save file normally, otherwise add ablation_1 to ablation_6 mark in the filename, not reflected in the folder name but in the specific filename
    ...
    json_path = os.path.join(
        out_dir,
        f"{ablation_suffix}Ours_epsilon_{args.epsilon}-{args.max_budget}_EmbInver_{args.EmbInver_K}_MaskInfer_{args.MaskInfer_K}.json"
    )
    csv_path = os.path.join(
        out_dir,
        f"{ablation_suffix}Ours_epsilon_{args.epsilon}-{args.max_budget}_EmbInver_{args.EmbInver_K}_MaskInfer_{args.MaskInfer_K}.csv"
    )
    time_json_path = os.path.join(
        out_dir,
        f"{ablation_suffix}Ours_epsilon_{args.epsilon}-{args.max_budget}_EmbInver_{args.EmbInver_K}_MaskInfer_{args.MaskInfer_K}_avg_pii_risk_addnoise_attack_time.json"
    )
    epsilon_json_path = os.path.join(
        out_dir,
        f"{ablation_suffix}Ours_epsilon_{args.epsilon}-{args.max_budget}_EmbInver_{args.EmbInver_K}_MaskInfer_{args.MaskInfer_K}_avg_epsilon_S.json"
    )
    avg_attack_results_path = os.path.join(
        out_dir,
        f"{ablation_suffix}Ours_epsilon_{args.epsilon}-{args.max_budget}_EmbInver_{args.EmbInver_K}_MaskInfer_{args.MaskInfer_K}_avg_attack_results.json"
    )
    ...
    # Calculate and save average values of attack results
    avg_attack_results = {
...
def batch_inference(texts, tokenizer, model, device, top_k=5):###Can be used directly in different baselines or different approaches, no need to modify this function
    """
...
def mask_token_inference_attack_subword_topk_batch(###Can be used directly in different baselines or different approaches, no need to modify this function
        Mask_success_words,
...
def main():
    ...
    # Load BERT model and tokenizer ##Needed for attack, remember to move to GPU
        ...
        try:
            ...
            for token, epsilon_i in zip(tokens, token_privacy_budgets):
                ...
                if epsilon_i is not None:
                    ...
                    # Embedding Inversion Attack---Different from non-attack situation
            ...
            # Calculate EmbInver success rate, total attack attempts is the length of tokens
