...
class DPNoiseAdder:
    ...
    def get_glove_replacement(self, token, is_high_risk, epsilon_effective):
        """
        Perform similar replacement for high-risk tokens.
        If the token is not in the GloVe dictionary, retain it directly.
        """
        ...
        # if token.lower() in self.special_tokens: # GloVe does not have special tokens
        #     return [token], [1.0]
        ...
        embeddings = self.embedding_matrix  # Tensor already on the device
        word2id = self.word2id
        id2word = self.id2word
        
        if token in word2id:
            idx = word2id[token]
            word_vector = embeddings[idx].unsqueeze(0)  # (1, 300)
            ...
            # Use torch.topk to improve efficiency and get the top K similarities and indices
            ...
            # Use exp to calculate probability distribution and normalize
            tmp = np.exp(epsilon_effective * new_sim_Cos_list / 2)
            norm = tmp.sum()
            if norm == 0:
                # Prevent division by zero, set uniform probability
                p = np.full_like(tmp, 1.0 / len(tmp))
            else:
                p = tmp / norm

            # Convert results to lists and save
            sim_words = []
            valid_p = []
            for i, prob in zip(top_indices, p):
                if i < len(id2word):
                    ...
                    if word:  # Ensure word is not None
                        ...
                        print(f"Warning: No word found for index {i}.")  # This situation did not occur
                else:
                    print(f"Warning: Index {i} is out of range.")  # This situation did not occur        
                if len(sim_words) == K:
                    break
            ...
            if len(sim_words) == K:
                ...
                print(f"Warning: Only {len(sim_words)} similar words found for '{token}'.")  # This situation did not occur
                # If less than top_k, supplement with the original word
        ...
        else:  # If not in the dictionary, retain the original word
            return [token], [1.0]


    def select_replacement(self, token, sim_words, probabilities):
        """
        Select a replacement word from candidate replacement words based on probabilities
        """
        if any(np.isnan(probabilities)):
            print(f"Skipping word '{token}' due to NaN in probabilities")  # This situation did not occur
            return token
        try:
            new_token = np.random.choice(sim_words, p=probabilities)
        except ValueError as ve:
            print(f"Error for token '{token}': {ve}")  # This situation did not occur
            return token
        return new_token